...
$stmt = $pdo->prepare("
    ...
    LEFT JOIN trainer_bookings b ON s.id = b.slot_id AND b.status IN ('pending', 'confirmed')
...
foreach ($holidaysRaw as $h) {
    $tidsStr = $h['trainer_ids'] ?? '[]';
    $tids = json_decode($tidsStr ?: '[]', true) ?: [];
...
<div class="av-page">
    ...
    <div class="av-board" id="avBoard">
        ...
        <div class="day-col" data-date="<?= $d ?>">
            ...
            <?php if ($hasHol): ?>
                <div style="background: linear-gradient(135deg, #fef2f2, #fee2e2); border: 1.5px dashed #fca5a5; border-radius: 12px; padding: 1.5rem 1rem; text-align: center; color: #ef4444; margin-top: 0.25rem; display: flex; flex-direction: column; align-items: center; justify-content: center; min-height: 140px; transition: all 0.2s ease;">
                    <i class="fa-solid fa-umbrella-beach mb-2" style="font-size: 2rem; opacity: 0.7;"></i>
                    <span style="font-size: 0.75rem; font-weight: 800; text-transform: uppercase; letter-spacing: 0.05em;"><?= htmlspecialchars($holidays[$d]) ?></span>
                </div>
            <?php else: ?>
                <?php if (empty($dSlots)): ?>
                    ...
                    <div class="slot-card <?= $statusClass ?>" data-id="<?= $slot['id'] ?>" data-state="<?= $slot['status'] ?>" title="Click to edit" onclick="handleClick(this)">
                        <div style="display:flex;justify-content:space-between;align-items:center;gap:.35rem;">
                            <div style="min-width:0;">
                                ...
                                <div class="sc-dur" style="font-size:.65rem;color:#64748b;margin-bottom:.2rem;">to <?= date('h:i A', strtotime($slot['end_time'])) ?></div>
                                <div class="sc-badge <?= $statusClass ?>">
                                    <i class="fa-solid fa-circle" style="font-size:.35rem;"></i> <?= ucfirst($slot['status']) ?>
                                </div>
                            </div>
                        ...
                        <?php if ($statusClass === 'booked'): ?>
                            <div style="font-size:.65rem;color:#ef4444;font-weight:700;margin-top:.4rem;padding-top:.3rem;border-top:1px solid rgba(239,68,68,0.2);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">
                                <i class="fa-solid fa-user me-1"></i> <?= htmlspecialchars($slot['client_name'] ?? 'Client Booked') ?>
                            </div>
                        <?php endif; ?>
                    </div>
                    <?php endforeach; ?>
                <?php endif; ?>
                <button class="add-slot-btn mt-1" onclick="openAddSlot('<?= $d ?>')">
...
function renderTimelineMarkup(date) {
    ...
    if (!slots.length) return `<div class="dv-empty"><i class="fa-regular fa-calendar-xmark"></i>No slots for this date</div>`;

    const icons = { available: 'fa-circle-check', booked: 'fa-user-clock', blocked: 'fa-ban' };

    return slots.map(slot => {
        const isBooked  = slot.status === 'booked';
        const isBlocked = slot.status === 'blocked';
        const chk = slot.status === 'available' ? 'checked' : '';
        const icon = icons[slot.status] || 'fa-circle';
